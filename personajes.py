...
class ProyectilFuego:
    def __init__(self, x, y, objetivo, frames, dano=4, velocidad=5):
        self.x = float(x)
        self.y = float(y)
        ...
        direccion_x = objetivo.rect.centerx - self.x
        direccion_y = objetivo.rect.centery - self.y
    ...
    def imagen_orientada(self):
        imagen = self.frames[self.frame]
        angulo = math.degrees(math.atan2(-self.velocidad_y, self.velocidad_x))
        return pygame.transform.rotate(imagen, angulo + 90)
...
class Personaje:
    def __init__(self, x, y, imagen, vida=100, ataque=10, defensa=5, velocidad=None):
        self.x = x
        self.y = y
        self.imagen = imagen
    ...
    def configurar_swoosh(self, frames, duracion_frame=80):
    ...
    def obtener_rect_swoosh(self):
        if not self.swoosh_activo:
            return None
        imagen_swoosh = self.swoosh_frames[self.swoosh_frame]
    ...
    def dibujar_swoosh(self, ventana):
        ...
        if rect_swoosh is not None:
            ventana.blit(self.swoosh_frames[self.swoosh_frame], rect_swoosh)
    ...
    def movimiento(self, mover_arriba, mover_abajo, mover_izquierda, mover_derecha):
        if mover_arriba:
            self.y -= self.atributos["velocidad"]
        if mover_abajo:
            self.y += self.atributos["velocidad"]
        if mover_izquierda:
...
class Enemigo(Personaje):
    def __init__(self, x, y, imagen):
        ...
        self.frames_fb = []
        self.ultimo_ataque_fb = -2000
        self.cooldown_fb = 2000
        self.distancia_minima_ataque = 250

    def configurar_ataque_fb(self, frames):
    ...
    def atacar_con_fb(self, objetivo):
        ...
        tiempo_actual = pygame.time.get_ticks()
        if distancia <= self.distancia_minima_ataque and tiempo_actual - self.ultimo_ataque_fb >= self.cooldown_fb:
    ...
    def movimiento(self):
        # Movimiento simple del enemigo hacia la izquierda
